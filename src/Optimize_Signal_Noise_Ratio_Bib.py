...
class OptSignalNoiseRation:
    ...
    def calc_max_em_gain(self, max_t_exp, min_t_exp, allow_every_mode = 'n'):
        ...
        #print(self.sky_flux,self.star_flux, self.n_pix_star,self.dark_noise,max_t_exp)        
        if aux > max_fotons:
    ...
    # Esta função determina os modos de operação que atendem a uma SNR mínima    # Para cada modo de operação é calculado o em_gain máximo. Este ganho é utilizado para
    ...
    def calc_max_snr_min_snr(self):
        ...
        for mode in self.MOB.get_list_of_modes():            
            ...
            SNRC = snrc.SignalToNoiseRatioCalc(min_t_exp, em_mode, max_em_gain, hss, preamp, binn, self.ccd_temp, self.sky_flux, self.star_flux, self.n_pix_star, self.serial_number)
            SNRC.set_gain_value()
            SNRC.calc_RN()
            SNRC.calc_DC()
            SNRC.calc_SNR()
    ...
    def create_space(self, allow_every_mode='n'):
        i=0    
        self.remove_repeat_modes()                   
        space_all_modes = []        
        ...
        for mode in self.filtered_list:            
            ...
            if mode['em_mode'] == 0:
                #como o ganho nao tem influencia para em_mode=0, eu forço o zero.
                em_gain = hp.choice('em_gain_'+ str(i), [0]) 
            else:
                self.set_gain(mode['em_mode'], mode['hss'], mode['preamp'])                
                max_em_gain, new_t_exp = self.calc_max_em_gain(mode['max_t_exp'], mode['min_t_exp'], allow_every_mode)                
                if new_t_exp != 0:
                    #new_t_exp é o novo t_exp que um modo EM precisa para não ultrapassar 100 f/pix
                    # se esta novo valor for maior que zero, o MOB recebe um novo intervalo de t_exp
                    t_exp   = hp.uniform('t_exp_' + str(i), mode['min_t_exp'],new_t_exp)
                #allow_every_mode permite que modos que não atendam a condição de 100 f/pix façam parte
                # do espaço de estados do MOB. Uso esta opção para avaliar o espaço de estados inteiro
                # de forma aleatória
    ...
    def creat_log_parameters(self, path, file_base_name):
        ...
        for i in range(len(op_modes_list)):
            ...
            dic['em_gain'] = float(dic_em_gain[str(item)].pop(0))            
            if mode['hss'] == 0.1:
...
##    def calc_max_em_gain_3(self, t_exp, allow_every_mode = 'n'):
##        #Cálculo do ganho EM máximo permitido para cada modo. O cálculo é baseado na quantidade máxima de 100 fótons
##        # por pixel do CCD para a qual o modo EM é melhor que o Convencional.
##        # Esta função recebe o t_exp máximo de cada modo. Contudo, dentro de um mesmo modo, o ganho EM poderia ser maior
##        # quando a iteração escolher um t_exp menor. Talvez, esta seja uma limitação da biblioteca. Ainda não achei solução
##
##        #Caso o numero de fótons/pix seja maior que seja, eu calculo para qual t_exp o numero de fótons atende essa condição
##        max_fotons = 100
##        bias = self.bias_level
##        max_ADU = (2**16)*0.8
