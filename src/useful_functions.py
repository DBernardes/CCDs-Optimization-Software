...
def get_obs_setup(img_dir):
    ...
    for line in lines:        
        ...
        if 'Binning dos pixeis' in line[0]: binn_modes = [int(binn) for binn in line[1].split(',')]
        ...
        if 'Nome do experimento' in line[0]: file_base_name = line[1].replace(' ','')
        if 'Exportar configuracao para txt' in line[0]: export_arq = line[1].replace(' ','')
        if 'Exportar lista de imagens de loss' in line[0]: export_loss = line[1].replace(' ','')        
        if 'Exportar lista de imagens de bias ' in line[0]: export_bias = line[1].replace(' ','')
        
        
        if 'Utilizar pre-imagem (s/n)' in line[0]: use_pre_img = line[1].strip().replace(' ','')     
