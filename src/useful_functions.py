...
def get_obs_setup(img_dir):
    ...
    for line in lines:        
        line = line.split('=')
        if 'Signal-to-Noise ratio' in line[0]: snr = float(line[1])
        if 'Acquisition rate' in line[0]: acq_rate = float(line[1])
        if 'Object magnitude' in line[0]: obj_magnitude = float(line[1])
        if 'Sub-image modes' in line[0]: sub_img_modes = [int(sub_img) for sub_img in line[1].split(',')]
        if 'Binning modes' in line[0]: binn_modes = [int(binn) for binn in line[1].split(',')]
        if 'CCD serial number' in line[0]: serial_number = int(line[1])
        if 'CCD temperature' in line[0]: ccd_temp = float(line[1])        
        if 'Iterations number of the optimization' in line[0]: max_evals = int(line[1])
        if 'Experiment name' in line[0]: file_base_name = line[1].replace(' ','')
        if 'Export the opt mode to a txt file' in line[0]: export_arq = line[1].replace(' ','')
        if 'Export the list of the iterations' in line[0]: export_loss = line[1].replace(' ','')        
        if 'Export the list of the bias images' in line[0]: export_bias = line[1].replace(' ','')
        
        
        if 'Use a pre-image' in line[0]: use_pre_img = line[1].strip().replace(' ','')
        if use_pre_img == 'y':
            if 'Image name' in line[0]:
                pre_img_name = line[1].strip().replace(' ','')
                if '.fits' not in pre_img_name:pre_img_name+='.fits'        
            if 'Object coordinates' in line[0]:obj_coords = (int(line[1].split(',')[0]), int(line[1].split(',')[1]))
            if 'Bias image name' in line[0]:
                bias_img_name = line[1].strip().replace(' ','')
                if '.fits' not in bias_img_name:bias_img_name+='.fits'
            if 'Maximum sky radius' in line[0]: sky_radius = int(line[1]) 
        
        
    return  snr, acq_rate, obj_magnitude, sub_img_modes, binn_modes, serial_number, ccd_temp, max_evals, file_base_name, export_arq, export_loss, export_bias, use_pre_img, pre_img_name, obj_coords, bias_img_name, sky_radius
